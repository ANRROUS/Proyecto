...
const Servicios = () => {
    ...
    const columnas = [
        ...
        { field: "duracion", headerName: "DURACIÓN", flex: 1, minWidth: 100 },
        {
            field: "acción",
            headerName: " ",
            flex: 0.5,
    ...
    return (
        <Grid container style={{ height: "auto + 20px" }}>
            ...
            >
                ...
                <Grid item xs={12} md={6.5} sx={{ margin: "10px", height: "auto", boxShadow: "0 4px 8px rgba(0, 0, 0, 0.544)", borderRadius: "20px", padding: "40px", display: "flex", alignItems: "center" }}>
                    <Paper sx={{ height: "auto", width: "100%", padding: 2, display: "flex", flexDirection: "column" }}>
                        <Dialog open={open} onClose={handleClose}>
                            <DialogTitle>{nuevoServicio.id ? "Editar Servicio" : "Agregar Nuevo Servicio"}</DialogTitle>
                            <DialogContent>
                                <TextField label="Nombre" name="nombre" value={nuevoServicio.nombre} onChange={handleChange} fullWidth margin="normal" />
                                <TextField label="Precio" name="precio" type="number" value={nuevoServicio.precio} onChange={handleChange} fullWidth margin="normal" />
                                <TextField label="Duracion" name="duracion" value={nuevoServicio.duracion} onChange={handleChange} fullWidth margin="normal" />
                            </DialogContent>
                            <DialogActions>
                                <Button onClick={handleClose} color="secondary">Cancelar</Button>
                                <Button onClick={() => handleAgregarServicio(nuevoServicio.id ? "Guardar cambios" : "Agregar")} color="primary">
                                    {nuevoServicio.id ? "Guardar cambios" : "Agregar"}
                                </Button>
                            </DialogActions>
                        </Dialog>
                ...
                <Grid item xs={12} md={4.5} sx={{ margin: "10px", height: "auto", boxShadow: "0 4px 8px rgba(0, 0, 0, 0.544)", borderRadius: "20px", padding: "10px", display:"flex", justifyContent:"center", alignItems:"center"}}>
